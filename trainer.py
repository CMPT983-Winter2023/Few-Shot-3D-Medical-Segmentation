...
def random_aug(batch_target, valid=False, sw_batch_size=4):
    ...
    for target in batch_target:
        ...
        if 0 < n <= 13:
            text = torch.load(os.path.join(text_path, "_".join(str(x.item()) for x in mask) + ".pt"))
        elif n == 0:
...
def train_epoch(model, loader, optimizer, scaler, epoch, loss_func, args):
    ...
    for idx, batch_data in enumerate(loader):
        ...
        target, text = random_aug(target)
        #print(data.shape, target.shape)
        data, target, text = data.cuda(args.rank), target.cuda(args.rank), text.cuda(args.rank)
        #text = torch.load(batch_data["image_meta_dict"]["filename_or_obj"][0].replace("img", "text").replace("nii.gz", "pt")).cuda(args.rank)
        #text = to_medclip(target).cuda(args.rank)

        for param in model.parameters():
            param.grad = None
        with autocast(enabled=args.amp):
            logits, L_contrast = model(data, text)
            if args.contrast:
                loss = loss_func(logits, target) + alpha * L_contrast
            else:
...
def val_epoch(model, loader, epoch, acc_func, args, model_inferer=None, post_label=None, post_pred=None):
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
            ...
            #data, target = data.cuda(args.rank), target.cuda(args.rank)
            #print(target.shape)
            target, text = random_aug(target, valid=True, sw_batch_size=args.sw_batch_size)
            #print(target.shape, text.shape)
            data, target, text = data.cuda(args.rank), target.cuda(args.rank), text.cuda(args.rank)
            #text = torch.load(batch_data["image_meta_dict"]["filename_or_obj"][0].replace("img", "text").replace("nii.gz", "pt")).cuda(args.rank)
            #text = to_medclip(target)
            with autocast(enabled=args.amp):
                if model_inferer is not None:
                    logits = model_inferer(data, text_in=text)
                else:
                    #logits = model(data, text)
                    logits, _ = model(data, text)
