...
from model.CrossAttention import CrossModalTransformer

class Universal_model(nn.Module):
    def __init__(self, img_size, in_channels, out_channels, backbone = 'swinunetr', encoding = 'rand_embedding', attention=False):
        ...
        if self.encoding == 'rand_embedding':
            ...
            self.register_buffer('organ_embedding', torch.randn(out_channels, 32))
            self.text_to_vision = nn.Linear(32, 256)
    ...
    def forward(self, x_in):
        ...
        elif self.encoding == 'word_embedding' or self.encoding == 'one_hot':
        ...
        for i in range(b):
            if self.attention:
                task_encoding, x_feat = self.cross_attention(task_encoding.reshape(self.class_num,1,256), x_feat[i].unsqueeze(0).repeat(self.class_num,1,1,1,1).reshape(self.class_num,1,256))
                x_cond = torch.cat([x_feat.reshape(32, 256, 1, 1, 1), task_encoding.reshape(32, 256, 1, 1, 1)], 1)
            else:
